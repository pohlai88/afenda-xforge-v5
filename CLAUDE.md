...
packages/design             @xforge/design     shadcn/ui components + src/foundation/ (the ADL as data) + src/styles/ (its Level-2 CSS, entry index.css); tests/ hold the composite behaviour contracts and R11–R14
packages/typescript-config                     base.json → library.json (Bundler) → react-library.json (jsx); nextjs.json
```

- Import design code as `@xforge/design/components/<name>`, `@xforge/design/lib/utils` (`cn`), and the canonical stylesheet as `@xforge/design/styles/index.css` (only `apps/web/app/globals.css` imports it); contracts as `@xforge/contracts/<domain>/<file>`, `@xforge/contracts/ids`, `/errors`, `/sources`, `/fixtures/<file>`. These resolve through each package's `exports` **and** the `paths` in `apps/web/tsconfig.json` — keep both in sync when adding an export.
...
- Tailwind is compiled once, in `apps/web` (`postcss.config.mjs`), from `apps/web/app/globals.css` — which imports the canonical `@xforge/design/styles/index.css` (the ADL graph, R11/R12), wires next/font into the ADL reference tier, `@source`s the design package, and carries the **transitional shadcn bridge**: legacy component vocabulary (`bg-background`, `text-muted-foreground`, …) aliased onto ADL system roles plus restored Tailwind defaults, deleted line by line as `10-components/*` rewrites each component. Tailwind only inlines **string** `@import`s; `@import url(...)` is left for Turbopack, which cannot resolve it under pnpm's isolated `node_modules`. Biome's CSS formatter leaves the notation alone.
