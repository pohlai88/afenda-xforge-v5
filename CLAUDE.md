...
- `apps/web/components.json` + `packages/design/components.json` are the shadcn config (style `base-nova`, base `base`). Base UI, not Radix: polymorphism is the `render` prop, never `asChild` — but a Button never wraps a link (Base UI enforces button semantics): a link that must look like a button wears `buttonVariants` on the real `<Link>` (AF-CMP-COMMON-BUTTON-004); menu items take `onClick`; a `DropdownMenuLabel` must sit inside a `DropdownMenuGroup` or the popup throws; `Select.onValueChange` receives `string | null` and `SelectValue` needs `items` on the root to label the value on the server; the element form of `render` merges props automatically, the function form does not — combine with `mergeProps` (handlers chain right-to-left; `event.preventBaseUIHandler()` stops Base UI's own logic), and a styled part with no primitive behind it is `useRender` + `mergeProps` with `className` still routed through `cn` (see `components/badge.tsx`). Always run the CLI with `-c apps/web`; it writes into `packages/design` through the aliases.
