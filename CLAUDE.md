...
| Unit tests, all packages | `pnpm test` (91: contracts 12, design 27, web 52) |
...
- Theme: `next-themes` with `attribute="class"`; `apps/web/components/theme-provider.tsx` also binds the `d` hotkey; `components/theme-toggle.tsx` is its visible twin in the shell. Colours are oklch CSS variables in `globals.css`, exposed to Tailwind via `@theme inline`, and they are two concepts under one contract (ADR-013, `docs/color-system-v2.md`): light is warm paper (hue 85, `muted` below the ground, sidebar lighter), dark is a cool chassis (hue 264, `muted` above the ground, sidebar darker). A value changes in both blocks with its measurement in the comment beside it, never at a call site. `--ring` aliases `--primary` and `--input` is darker than `--border` on purpose: `packages/design/tests/tokens.test.ts` holds every `x`/`x-foreground` pair — at rest and hovered — to 4.5:1 and the focus ring and field boundary to 3:1 on every ground in both themes (R1), keeps every adjacent fill pair at least 0.015 apart in Oklab (R8), refuses any oklch outside sRGB (R9) and a self-referencing `@theme` alias (R5); `tests/motion.test.ts` pins the reduced-motion block (R10).
