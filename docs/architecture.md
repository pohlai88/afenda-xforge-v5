...
- **State grammar, decided once.** Hover and press derive from the role's fill at the alpha base-nova ships (`hover:bg-primary/80`); soft tones are persistent tinted pairs and are measured at rest; selected is a component-specific role treatment; disabled is a component state; focus is the focus-indicator grammar, never a fill state. No `*-hover`, `*-pressed` or `*-focus` tokens; M3's content-colour overlay and v4's opaque fills are both declined, here, with this sentence as the reason (R6).
