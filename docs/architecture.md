...
- **The ADL schemes are measured, and the language holds its own integrity (R13, R14).** R13 (`packages/design/tests/schemes.test.ts`) is the ADL twin of R1/R8/R9: across all six schemes (light/dark × standard/medium/high contrast) every text pair *derived from the roles themselves* holds 4.5:1 — deriving rather than hand-listing is what caught `on-primary-fixed-variant` at 3.85:1 on the dim tone the hand-list missed — outline, focus and error boundaries hold 3:1 on every surface, hovered fills stay legible under the 4% state layer, data marks clear 3:1 with monotonic sequential ramps, ladder steps stay 0.015 apart in Oklab, and every reference oklch is inside sRGB. One structural waiver, ratcheted at its measured 0.0095 and refused the moment it clears the floor: `surface-container-lowest` vs `-low`, because only 0.0188 of headroom exists between white and the ground. The measurements it minted with: the light ladder re-stepped (`high`/`highest` to slate-93/slate-90), categorical-8 moved to a plum family in both themes (teal could not clear 3:1 and stay tellable from categorical-1), `on-primary-fixed-variant` to blue-40. **The derivation boundary (2026-09-05, held against Prototyper UI's derive-vs-define table):** the state-layer composite is the one admitted runtime derivation, licensed by R13's hover measurement; `color-mix()` values are declined because a value the file doesn't contain has nowhere to carry its measurement — and their own test replicates why (ramp derivation collapses for light colours, exactly where our ladder's ratcheted waiver sits; foregrounds can't be derived at all). A need that looks derivation-shaped is computed at design time, written into the file with its measurement, and held by R13. The boundary binds *measured* values: geometry carries no measurement, so `calc()` derivation is admissible there — but not before a consumer (the tenant shape contract), and not over steps that are designed values rather than linear offsets. R14 (`language.test.ts`) holds the language to itself: every domain identity block equals its registry row in `00-principles`, and every `AF-*` rule ID across the foundation — and the block contracts in `src/blocks/*/`, where the component domain lives — matches the grammar and is defined exactly once; its planted defect replays the day `05-layout.ts` was a copy of `04-geometry.ts` carrying all 104 `AF-GEO` rules twice.
- **A block is held equal to its contract (R15).** `packages/design/tests/blocks.test.ts`: for every contract, the block stamps exactly the declared anatomy, implements exactly the admitted variants and sizes, draws exactly the admitted colour roles — both directions, so the admitted list cannot rot — names behaviour rules that exist, keeps its `AF-CMP-<CODE>` rules inside the grammar, and points at a live exemplar that uses the block; contracts and blocks are paired both ways, so neither can exist alone. A block is **defined, not rendered**, and its folder carries the component's whole truth under its own name — `<name>-contract.ts`, `<name>-manifest.ts` (parts keyed by the `data-slot` each stamps, every part's base, variants and sizes as ADL utility literals) and `<name>.tsx` deriving every class from that data; never `index`, never a numbered file (the naming law is a check with a planted defect, and the globs can only pair correctly named files); a `data-slot` string literal in a render is itself a finding, and the render names its part as a type-checked key reference into the definition. The part keys are the anatomy, held equal to the contract's in both directions — generalised from the single-slot shape 2026-09-05 (off Prototyper UI's progressive-disclosure review) so composite blocks land on the settled grammar and a boolean-gated part has nowhere to live; variant/size admission is checked against the union across parts, and a block admitting states must compose `af-interactive` in at least one part's base. Minted 2026-09-05 with Button, the first block: state-layer hover via the `state-layer-*` utility, af-interactive for focus/disabled/invalid, semantic geometry throughout. Two build facts landed with it: the class-bearing ADL sheets moved into `layer(components)` (an unlayered `.af-interactive` beats every utility a block composes with it), and the ADL's theme wipe narrowed to `--color-*: initial` in its own `@theme` block — the wildcard wipe had killed the ADL's own tokens (the formatter sorts a shared block) and forced the bridge to resurrect Tailwind defaults piecemeal.
...
- **The ADL is the compiled system (2026-09-05).** `apps/web/app/globals.css` is the app's only stylesheet: it imports the canonical `@xforge/design/styles/index.css`, provisions next/font into the ADL reference tier, and carries the transitional shadcn bridge — legacy vocabulary aliased onto ADL roles (every pair transitively measured by R13), deleted line by line as the blocks rewrite each component. The legacy `globals.css` and its guards retired with it: R1/R8/R9's subjects are now measured by R13, R5 and R10 moved with the bridge into `apps/web/tests/design-vocabulary.test.ts`, and R4 now scans the bridge. The paragraphs above describing `tokens.test.ts` and the ADR-013 values document that era's decisions and measurements.
...
- **No boolean-prop proliferation.** Behaviour is composed from parts, not switched on. **Compound components share context**; the provider is the only place that knows how state is managed. **Children over render props**; `cva` for the variant axes a primitive genuinely has. **React 19**: `ref` is a prop; `use()` over `useContext()`. **Progressive disclosure has no boolean middle rung** (sharpened against Prototyper UI's principle 7, 2026-09-05): Level 1 is a composed block that just works, Level 3 is the parts, and more control is reached by descending to composition — never by a visibility boolean, because a boolean-gated `data-slot` makes the anatomy conditional, which anatomy-as-data (R2) cannot express. Behaviour booleans owned by the primitive (`focusableWhenDisabled`, `nativeButton`) are a different class and stay. The vendored dialog carries upstream's `showCloseButton` on Content and Footer — quarantined by the vendored-layer style override; the dialog block's contract refuses it (AF-CMP-DIALOG-004, landed 2026-09-05: the close affordance is anatomy, always stamped).
- **A destructive act confirms at the definition, not the call site** (Compose's action-level `confirm`, adopted 2026-09-05): the menu item or button only opens the alert dialog — the ellipsis in its label says so — and the mutation fires solely from the dialog's confirm action; no call site invokes a destructive mutation directly. The remove-member flow is the exemplar (`features/members/components/member-row-actions.tsx`, with the cancel-removes-nobody test beside it); review enforces the rule until the alert-dialog block contract carries it.
...
| 016 | Governed components are **blocks**: `packages/design/src/blocks/<name>/` — one folder holding the component's whole truth (`<name>-contract.ts`, `<name>-manifest.ts`, `<name>.tsx`; every file named by the component, never `index` — R15 refuses a stray name), consumed as `@xforge/design/blocks/<name>` via explicit per-block entries in the package `exports` map and `apps/web/tsconfig.json` paths | Accepted 2026-09-05; contracts relocated from `foundation/10-components/` into the block folders 2026-09-05 | `src/components/` is the shadcn CLI's write target and a refresh may overwrite any file in it — deliberately, so `shadcn diff` stays meaningful — which is why governed code cannot live there; a block lands with its contract (the SSOT for what the component is and may be; the manifest is the SSOT for what it draws — the overlap is the check, double-entry style), writes ADL vocabulary only, retires its shadcn-bridge lines from `apps/web/app/globals.css` (R4 confirms), and is held equal to its contract by R15, minted with the first block |
