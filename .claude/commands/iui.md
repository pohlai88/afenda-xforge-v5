---
description: shadcn-studio Inspire UI — browse studio blocks as source material, then synthesise a governed block in the ADL
---

Run the shadcn-studio **Inspire UI** workflow for: $ARGUMENTS

Call `mcp__shadcn-studio__get-inspire-instructions` first and follow its
METHOD: analyse blocks for layout DNA, component combinations and UX
mechanics, then **synthesise** — inspiration is source material, never a
drop-in.
...
4. Inspiration remains configured-but-unadopted until a specific block passes
   the checks: `pnpm check`, `pnpm typecheck`, `pnpm test` — and the e2e when
   a screen changed.
